...
import { Calculator, Receipt, DollarSign, ArrowRight, AlertCircle, FileText, Percent, Zap, Scale } from 'lucide-react'
...
const WithholdingRoyalties = () => {
  ...
  const commonDTARates = [
    { value: '15', label: 'Standard Rate (15%)', description: 'Most countries' },
  ...
  const handleCalculate = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/calculate/withholding/royalties`,
        payload
      )
      
      setResults({
        taxDue: response.data.taxDue,
        royaltyAmount: payload.value,
          ...
          <div className="bg-white rounded-2xl p-8 border border-[#FFD700] shadow-lg">
            <div className="flex items-center justify-center gap-4 mt-4 mb-6">
              <div className="p-3 bg-[#1ED760]/10 rounded-2xl">
                <FileText className="w-8 h-8 text-[#1ED760]" />
              </div>
            ...
            <p className="text-xl text-[#0F2F4E]/80 max-w-2xl mx-auto">
              Calculate withholding tax on royalty payments to non-residents. Consider applicable Double Taxation Agreement rates.
            </p>
          </div>
        </motion.div>
          ...
          >
                ...
                >
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-[#0F2F4E]">Royalty Amount:</span>
                      <span className="text-[#0F2F4E] font-semibold">
                        ${(results.royaltyAmount || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    </div>
            ...
            <div className="bg-white rounded-2xl border border-[#FFD700] p-6 shadow-lg">
              ...
              <div className="space-y-3 text-sm text-[#0F2F4E]">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-[#0F2F4E]/70">5-10% with DTA</span>
                </div>
                <div className="flex justify-between">
            ...
            <div className="bg-white rounded-2xl border border-[#FFD700] p-6 shadow-lg">
              ...
              <ul className="space-y-3 text-sm text-[#0F2F4E]">
                ...
                <li className="flex items-start gap-2">
                  <Zap className="w-4 h-4 text-[#1ED760] mt-0.5 flex-shrink-0" />
                  <span>Withhold tax when making royalty payments to non-residents</span>
                </li>
                <li className="flex items-start gap-2">
                  <Zap className="w-4 h-4 text-[#1ED760] mt-0.5 flex-shrink-0" />
                  <span>File withholding tax returns by the 10th of following month</span>
                </li>
                <li className="flex items-start gap-2">
                  <Zap className="w-4 h-4 text-[#1ED760] mt-0.5 flex-shrink-0" />
                  <span>Maintain documentation of DTA applicability</span>
                </li>
              </ul>
            </div>

            {/* Treaty Countries Info */}
            <div className="bg-white rounded-2xl border border-[#FFD700] p-6 shadow-lg">
              <div className="flex items-center gap-2 mb-3">
                <Scale className="w-4 h-4 text-[#1ED760]" />
                <h3 className="text-lg font-bold text-[#0F2F4E]">
                  Treaty Countries
                </h3>
              </div>
              <div className="text-sm text-[#0F2F4E] space-y-3">
                <p className="text-[#0F2F4E]/80">Zimbabwe has DTAs with multiple countries including:</p>
            ...
            <div className="bg-white rounded-2xl border border-[#FFD700] p-6 shadow-lg">
              ...
              <ul className="space-y-2 text-sm text-[#0F2F4E]">
                ...
                <li className="flex items-start gap-2">
                  <div className="w-1.5 h-1.5 bg-[#1ED760] rounded-full mt-1.5 flex-shrink-0" />
                  <span>Maintain records for 7 years</span>
                </li>
              </ul>
            </div>
          </motion.div>
...
export default WithholdingRoyalties